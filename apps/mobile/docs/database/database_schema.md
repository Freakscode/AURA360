...
-- Tier limitado a valores enumerados
ALTER TABLE public.app_users 
...
- `tier`: "Membership plan for the user (free or premium)."
...
class AppUser {
  ...
  final int? appUserId;            // id interno (bigserial)
  final String email;              // email
  final UserTier tier;             // tier (enum)
  final GlobalRole role;           // role_global (enum)
  final bool isIndependent;        // is_independent
  final BillingPlan billingPlan;   // billing_plan (enum)
  ...
  bool get operatesIndependently => isIndependent;
  bool get isAdmin => role.isAdminSistema || role.isAdminInstitucion || role.isAdminInstitucionSalud;
...
**Notas**:
- `id` sigue referenciando `auth_user_id`, mientras que `appUserId` expone el identificador interno requerido para memberships y relaciones clínicas.
- Los nuevos enums `GlobalRole` y `BillingPlan` sincronizan los valores controlados por la base para habilitar flujos B2B/B2C/B2B2C.
- `isIndependent` ayuda a construir el contexto de acceso cuando el usuario opera fuera de una institución.
